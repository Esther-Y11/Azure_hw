...
#AZURE
SUBSCRIPTION_KEY = os.getenv("azsubscription_key")
ENDPOINT = os.getenv("azendpoint")
CV_CLIENT = ComputerVisionClient(
    ENDPOINT, CognitiveServicesCredentials(SUBSCRIPTION_KEY)
)

FACE_KEY = os.getenv("azface_key")
FACE_END = os.getenv("azface_end")
...
IMGUR_CONFIG ={
            "client_id":os.getenv("imclient_id"),
            "client_secret":os.getenv("imclient_secret"),
            "access_token":os.getenv("imaccess_token"),
            "refresh_token":os.getenv("imrefresh_token")
}
...
def handle_content_message(event):
    ...
    if name != "":
        ...
        if len(plate) > 0:
            ...
            output, title = azure_describe(link)
    ...
    bubble["body"]["contents"][1]["contents"][1]["text"] = title
    LINE_BOT.reply_message(
